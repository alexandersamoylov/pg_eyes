...
/*
Изменена метрика: pg_stat_activity.active_time
    Исключены сессии с application_name = "DBA Task" и локальным client_addr
    при получении значений.
Добавлены метрики:
    pg_stat_activity.dba_task - Кол-во активных сессий для "DBA Task"
    pg_stat_activity.dba_task_time - Наибольшее время выполнения для "DBA Task"
    wal_written_b - Объем данных, записанных в WAL
*/
...
BEGIN
    ...
    SELECT count(1) AS total,
        ...
        coalesce(max(round(extract(epoch FROM age(statement_timestamp(),
            ...
            AND NOT (application_name = 'DBA Task' AND client_addr IS NULL)
            ), 0) AS active_time,
        ...
        count(*) FILTER (WHERE state = 'active'
            AND application_name = 'DBA Task' AND client_addr IS NULL)
            AS dba_task,
        coalesce(max(round(extract(epoch FROM age(statement_timestamp(),
            query_start))*1000)) FILTER (WHERE state = 'active'
            AND application_name = 'DBA Task' AND client_addr IS NULL), 0)
            AS dba_task_time
        INTO v_total,
