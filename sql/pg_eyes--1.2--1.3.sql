...
/*
Изменены метрики: pg_stat_activity.active_time, pg_stat_activity.dba_task,
    pg_stat_activity.dba_task_time.  При расчете случая с
    application_name = "DBA Task" убрано ограничение client_addr IS NULL
*/
...
BEGIN
    ...
    SELECT count(1) AS total,
        ...
        coalesce(max(round(extract(epoch FROM age(statement_timestamp(),
            query_start))*1000)) FILTER (WHERE state = 'active'
            AND application_name = 'DBA Task'), 0)
            AS dba_task_time
        INTO v_total,
