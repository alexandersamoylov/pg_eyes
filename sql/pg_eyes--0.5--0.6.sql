...
/*
Изменены метрики: pg_stat_activity.active_time, pg_stat_activity.xact_time
    Исключены сессии с autovacuum при получении значений.
Изменены названия метрик waiting, waitштп_1 и waiting_time на соответствующие из
    версии 1.0+: wait_lock, wait_lock_1s и wait_lock_time.
*/
...
BEGIN
    ...
    SELECT count(1) AS total,
        ...
        coalesce(max(round(extract(epoch FROM age(statement_timestamp(),
            ...
            AND query NOT LIKE 'autovacuum:%'), 0) AS active_time,
